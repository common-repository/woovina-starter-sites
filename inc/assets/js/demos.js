(function($) {
	...
	$(document).ready(function() {
		woovinaDemoImport.init();
	});

	var woovinaDemoImport = {
		...
		init: function() {
			...
			$('.woovina-search-input').on('keyup', function() {
				if(0 < $(this).val().length) {
					// Hide all items.
					$('.woovina-demo-wrap .themes').find('.theme-wrap').hide();

					// Show just the ones that have a match on the import name.
					$('.woovina-demo-wrap .themes').find('.theme-wrap[data-name*="' + $(this).val().toLowerCase() + '"]').show();
				} else {
					$('.woovina-demo-wrap .themes').find('.theme-wrap').show();
				}
			});

			// Prevent the popup from showing when the live preview button
			$('.woovina-demo-wrap .theme-actions a.button').on('click', function(e) {
			   e.stopPropagation();
			});

			// Get demo data
			$('.woovina-open-popup').click(function(e) {
				e.preventDefault();
			...
			$(document).on('click' 						, '.install-woovina-package', this.installWooVinaNow);
		},
		...
		categoriesFilter: function() {
			...
			var $items 				= $('.woovina-demo-wrap .themes').find('.theme-wrap'),
				fadeoutClass 		= 'woovina-is-fadeout',
				fadeinClass 		= 'woovina-is-fadein',
			...
			var fadeIn = function (category, dfd) {
				...
				$items.filter(filter).show().addClass('woovina-is-fadein');
			...
			$('.woovina-navigation-link').on('click', function(event) {
				...
				var $container = $('.woovina-demo-wrap .themes');
		...
		getDemoData: function(demo_name) {
			...
			// Get import data
			$.ajax({
				url: woovinaDemos.ajaxurl,
				type: 'get',

				data: {
					action: 'woovina_ajax_get_import_data',
					demo_name: demo_name,
					security: woovinaDemos.woovina_import_data_nonce
				},
			...
			$.ajax({
				...
				data: {
					action : 'woovina_ajax_get_demo_data',
					demo_name: demo_name,
					demo_data_nonce: woovinaDemos.demo_data_nonce
				},
		...
		runPopup: function(data) {
			...
			// Show popup
			$('#woovina-demo-popup-wrap').fadeIn();
			$(data.responseText).appendTo($('#woovina-demo-popup-content'));

			// Close popup
			$('.woovina-demo-popup-close, .woovina-demo-popup-overlay').on('click', function(e) {
				e.preventDefault();
			...
			$('.woovina-plugins-next').on('click', function(e) {
				e.preventDefault();
				
				// Hide step one
				$('#woovina-demo-plugins').hide();

				// Display step two
				$('#woovina-demo-import-form').show();

			});

			// if clicked on import data button
			$('#woovina-demo-import-form').submit(function(e) {
				e.preventDefault();

				// Vars
				var demo 	= $(this).find('[name="woovina_import_demo"]').val(),
					nonce 	= $(this).find('[name="woovina_import_demo_data_nonce"]').val(),
					contentToImport = [];
				...
				that.importContent({
					...
					isXML: $('#woovina_import_xml').is(':checked')
				});
			});
		...
		importContent: function(importData) {
			var that = this,
				...
				timerStart = Date.now(),
				ajaxData = {
					woovina_import_demo: importData.demo,
					woovina_import_demo_data_nonce: importData.nonce
				};

			this.allowPopupClosing = false;
			$('.woovina-demo-popup-close').fadeOut();
			...
			if(importData.contentToImport.length === 0) {				
				...
				setTimeout(function() {
					var error = $('.woovina-loader').find('.woovina-importing-failed');					
					if(error.length == 0) {
						$('.woovina-loader').hide();
						$('.woovina-last.woovina-success').show();
					}
					else {
						$('.woovina-last.woovina-error').show();
					}
				}, 1000);				
				...
				$.ajax({
					...
					type: 'post',
					data: {
						action: 'woovina_after_import',
						woovina_import_demo: importData.demo,
						woovina_import_demo_data_nonce: importData.nonce,
						woovina_import_is_xml: importData.isXML
					},
					complete: function(data) {}
				});

				this.allowPopupClosing = true;
				$('.woovina-demo-popup-close').fadeIn();
			...
			$('.woovina-import-status').append('<p class="woovina-importing">' + this.importData[ currentContent ]['loader'] + '</p>');
			...
			var ajaxRequest = $.ajax({
				...
				complete: function(data) {
					...
					if(data.status === 500 || data.status === 502 || data.status === 503) {
						$('.woovina-importing')
							.addClass('woovina-importing-failed')
							.removeClass('woovina-importing')
							.text(woovinaDemos.content_importing_error + ' '+ data.status);
					} else if(data.responseText.indexOf('successful import') !== -1) {
						$('.woovina-importing').addClass('woovina-imported').removeClass('woovina-importing');
					} else {
			...
			importingLimit = setTimeout(function() {
				...
				$('.woovina-importing')
					.addClass('woovina-importing-failed')
					.removeClass('woovina-importing')
					.text(woovinaDemos.content_importing_error);
			}, 15 * 60 * 1000);
		...
		closePopup: function() {
			...
			// Hide demo popup
			$('#woovina-demo-popup-wrap').fadeOut();

			// Remove content in the popup
			setTimeout(function() {
				$('#woovina-demo-popup-content').html('');
			}, 600);
		},
		...
		installNow: function(e) {
			...
			if('premium' == $type) {
				...
				type: 'POST',
				data: {
						action : 'woovina_ajax_install_plugin',
						slug: $slug,
						init: $init,
					},
				}).done(function(result) {
					$button.addClass('activate-now updating-message button-primary').removeClass('install-now').html(woovinaDemos.button_activating);
					...
					setTimeout(function() {
						$.ajax({
							...
							type: 'POST',
							data: {
								action : 'woovina_ajax_required_plugins_activate',
								init   : $init,
							},
						}).done(function(result) {
							if(result.success) {
								$button.removeClass('button-primary install-now activate-now updating-message')
									...
									.addClass('disabled')
									.text(woovinaDemos.button_active);
		...
		installWooVinaNow: function(e) {
			...
			$.ajax({
				...
				type: 'POST',
				data: {
					action : 'woovina_ajax_install_theme',					
				},
			}).done(function(result) {
				$button.removeClass('button-primary install-now activate-now updating-message')
					.attr('disabled', 'disabled')
		...
		activatePlugins: function(e) {
			...
			$button.addClass('updating-message button-primary').html(woovinaDemos.button_activating);
		...
		installSuccess: function(e, response) {
			...
			var $message = $('.woovina-plugin-' + response.slug).find('.button');
			...
			$message.removeClass('install-now installed button-disabled updated-message')
				...
				.html(woovinaDemos.button_activating);
		...
		pluginInstalling: function(e, args) {
			...
			var $card = $('.woovina-plugin-' + args.slug),
		...
		installError: function(e, response) {
			...
			var $card = $('.woovina-plugin-' + response.slug);
